...
impl Expr {
    pub fn eval(&self, variables: &HashMap<&str, Value>) -> bool {
        match self {
            ...
            Self::EqAnyCase(name, literal) => {
                Self::Eq(name.clone(), literal.clone()).eval(variables)
                    || match (variables.get(name.as_str()), literal) {
                        (Some(Value::Str(val)), Literal::Str(lit_str)) => {
                            val.to_lowercase() == lit_str.to_lowercase()
                        }
                        _ => true,
                    }
            }
